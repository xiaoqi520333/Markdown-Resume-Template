...
**秩益科技有限公司 — 后端开发实习生**　`2026-03 ~ 至今`

* 负责远程任务与节点占用闭环：基于 Redis（agent 前缀 + IP）感知节点在线/离线；删除节点时同步清理注册缓存，降低无效调度与脏数据风险。
* 因 YAML 改配依赖模块重启、耗时高，将配置改为数据库维护并按节点读库；避免频繁重启，配置拉取自测约 30ms/次。
* 迭代远程任务能力：启停、Windows/Linux 命令兼容、任务详情与进度展示；完善权限与按钮级鉴权；并参与一次约 20+ 文件规模的框架对齐重构。
...
* **缓存与中间件：** 熟悉 Redis、Caffeine + Redis 多级缓存；掌握 Redisson 分布式限流；熟悉 RabbitMQ 的异步解耦与削峰；了解 Kafka，在项目中经 AOP 采集操作信息后调用封装好的生产者异步发送消息参与日志落库链路。
